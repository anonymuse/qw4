"""Phase 0 fused routing payload validation for DS5-F002.
...
ROUTING_MANIFEST_SCHEMA_VERSION = "ds5.fused_routing_manifest.v1"
ROUTING_ARTIFACT_SCHEMA_VERSION = "ds5.fused_routing_artifact.v1"
FEATURE_ID = "DS5-F002"
...
class RoutingPlanValidationError(ValueError):
    """Raised when a DS5-F002 routing scaffold violates the Phase 0 contract."""
...
def build_routing_artifact(manifest: dict[str, Any], *, manifest_path: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "validation": {
            "status": "pass",
            "checked_invariants": [
                "exact DS5-F002 routing record field names",
...
def format_artifact_summary(artifact: dict[str, Any]) -> str:
    ...
    lines = [
        f"{FEATURE_ID} fused routing scaffold: PASS",
        ...
        "It does not load Qwen weights, implement fused routing, or report copy-count telemetry.",
    ]
    return "\n".join(lines)
...
def format_finding_summary(artifact: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "This summary validates routing payload shape, Qwen top-8 record constraints, DS5 B/C target-node "
        "bounds, and zero-copy assumptions as machine-readable planning data only. It does not load Qwen "
        "weights, tokenizer assets, Metal kernels, a KV allocator, or a primary-weight loader.",
        "",
        "## Payload Contract",
        "",
        "| Field | Value |",
